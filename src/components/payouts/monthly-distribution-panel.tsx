...
export function MonthlyDistributionPanel({ range }: { range: AdminDateRange }): React.JSX.Element {
  ...
  useEffect(() => {
    void load().catch(() => notify.error('Could not load monthly distributions'));
  }, [load]);
  useEffect(() => {
    setPage(1);
  }, [range]);
  const open = async (id: string) => {
    setLoading(true);
    try {
      setDetail(await distributionBatchService.get(id));
    } catch (error) {
      notify.error(error instanceof Error ? error.message : 'Could not load distribution');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="app-surface overflow-x-auto rounded-2xl border">
        <table className="w-full text-left text-sm">
          <thead className="border-b bg-muted/30">
            <tr>
              {[
                ...
                <th key={label} className="px-4 py-3 font-semibold text-muted-foreground">
                  {label}
                </th>
              ))}
            </tr>
          ...
          <tbody className="divide-y">
            {items.map((item) => (
              <tr key={item._id}>
                <td className="px-4 py-4 font-medium">{item.opportunityId.title}</td>
                <td className="px-4 py-4">{date(item.scheduledFor)}</td>
                <td className="px-4 py-4">{item.cycleNumber}</td>
                <td className="px-4 py-4">{item.ownershipCount}</td>
                <td className="px-4 py-4 text-brand">
                  {item.status === 'AWAITING_AMOUNT'
                    ? 'Awaiting amount'
                    : money(item.totalActualReturnMinorUnits)}
                </td>
            ...
            {!items.length && (
              <tr>
                <td colSpan={7} className="p-8 text-center text-muted-foreground">
                  No monthly distributions are due in this period.
                </td>
              </tr>
            )}
          </tbody>
...
function DistributionDrawer({
  ...
  detail: DistributionBatchDetail;
  close: () => void;
  updateDetail: (updated: DistributionBatchDetail) => void;
  refresh: () => Promise<void>;
}) {
  const [method, setMethod] = useState<
    'RATE_PERCENT' | 'AMOUNT_PER_UNIT' | 'TOTAL_DISTRIBUTION_AMOUNT'
  >(
  ...
  const [rate, setRate] = useState(
    detail.batch.actualRatePercent ? String(detail.batch.actualRatePercent) : '',
  );
  const [manualReturns, setManualReturns] = useState<Record<string, string>>(() =>
    Object.fromEntries(
      detail.allocations.map((item) => [item._id, String(item.returnMinorUnits / 100)]),
    ),
  );
  ...
  const calculate = async () => {
    ...
    if (!Number.isFinite(value) || value < 0)
      return notify.error('Enter a valid actual distribution amount');
    setBusy(true);
    try {
      ...
      notify.error(error instanceof Error ? error.message : 'Could not calculate allocations');
    } finally {
      setBusy(false);
    }
  };
  const review = async (status: 'APPROVED' | 'REJECTED') => {
    ...
    if (
      status === 'APPROVED' &&
      !window.confirm(
        `Approve ${money(detail.batch.totalActualReturnMinorUnits)} across ${detail.allocations.length} members using their selected profit treatment?`,
      )
    )
      return;
    setBusy(true);
    try {
      await distributionBatchService.review(detail.batch, status, note);
      notify.success(
        status === 'APPROVED' ? 'Monthly profits processed for members' : 'Distribution rejected',
      );
      await refresh();
    } catch (error) {
      notify.error(error instanceof Error ? error.message : 'Could not review distribution');
    } finally {
      setBusy(false);
    }
  };
  const saveManualReturns = async () => {
    ...
    if (
      allocations.some(
        (item) => !Number.isFinite(item.returnMinorUnits) || item.returnMinorUnits < 0,
      )
    )
      return notify.error('Enter a valid actual return for every member');
    setBusy(true);
    try {
      const updated = await distributionBatchService.setManualAllocations(
        detail.batch,
        allocations,
      );
      notify.success(
        'Custom member returns saved. These are now the amounts that will be credited.',
      );
      updateDetail(updated);
    } catch (error) {
      notify.error(error instanceof Error ? error.message : 'Could not save custom returns');
    } finally {
      setBusy(false);
    }
  };
  const ready = detail.batch.status === 'READY_FOR_APPROVAL';
  const editable = ['AWAITING_AMOUNT', 'READY_FOR_APPROVAL', 'REJECTED'].includes(
    detail.batch.status,
  );
  return (
      ...
      <aside className="h-full w-full max-w-3xl overflow-y-auto bg-background shadow-2xl">
        <header className="sticky top-0 z-10 flex items-start justify-between border-b bg-background/95 p-6 backdrop-blur">
          <div>
            ...
            <h2 className="mt-1 font-sans text-2xl font-semibold">
              {detail.batch.opportunityId.title}
            </h2>
            <p className="mt-1 text-sm text-muted-foreground">
              Due {date(detail.batch.scheduledFor)} · cycle {detail.batch.cycleNumber}
            </p>
          </div>
        ...
        <div className="space-y-6 p-6">
          <section className="rounded-2xl border p-5">
            <h3 className="font-sans text-lg font-semibold">Set actual monthly earnings</h3>
            <p className="mt-1 text-sm text-muted-foreground">
              Projected figures are disclosures only. Enter a realised rate, amount per unit, or one
              total amount to share across the opportunity’s economic units.
            </p>
            {editable && (
              <div className="mt-4 grid gap-3 sm:grid-cols-[220px_1fr_auto]">
                ...
                >
                  <option value="AMOUNT_PER_UNIT">Actual amount per unit</option>
                  <option value="RATE_PERCENT">Actual rate on capital</option>
                  <option value="TOTAL_DISTRIBUTION_AMOUNT">Total amount to distribute</option>
                </select>
                <input
                  type="number"
                  min="0"
                  step="0.01"
                  value={method === 'RATE_PERCENT' ? rate : amount}
                  onChange={(event) =>
                    method === 'RATE_PERCENT'
                  ...
                  placeholder={
                    method === 'RATE_PERCENT'
                      ? 'e.g. 3.5%'
                      : method === 'TOTAL_DISTRIBUTION_AMOUNT'
                        ? 'e.g. ₦3,000,000'
                        : 'e.g. ₦71,428.57'
                  }
                  className="rounded-xl border bg-background px-3 py-2.5 text-sm"
                ...
                <button
                  ...
                  className="rounded-xl bg-brand px-4 py-2.5 text-sm font-semibold text-brand-foreground disabled:opacity-50"
                >
          ...
          <section className="rounded-2xl border">
            <div className="border-b p-5">
              <h3 className="font-sans text-lg font-semibold">Member allocation preview</h3>
              <p className="mt-1 text-sm text-muted-foreground">
                Use the calculator above for a quick estimate, or enter the exact amount each member
                should receive below. Each member&apos;s selected profit treatment is applied when
                this distribution is approved.
              </p>
            </div>
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y">
                  {detail.allocations.map((item) => {
                    const compounds =
                      item.ownershipId.rolloverElection === 'COMPOUND' ||
                      (item.ownershipId.rolloverElection == null &&
                        item.ownershipId.rolloverCompoundsReturns);
                    return (
                      <tr key={item._id}>
                        ...
                        <td className="px-4 py-3">{item.ownershipId.units}</td>
                        <td className="px-4 py-3">{money(item.principalBeforeMinorUnits)}</td>
                        <td className="px-4 py-3 text-brand">
                          {editable ? (
                            <input
                              aria-label={`Actual return for ${item.userId.name}`}
                              type="number"
                              min="0"
                              step="0.01"
                              value={manualReturns[item._id] ?? ''}
                              onChange={(event) =>
                                setManualReturns((current) => ({
                                  ...current,
                                  [item._id]: event.target.value,
                                }))
                              }
                              className="w-32 rounded-lg border bg-background px-2 py-1.5 text-sm text-foreground"
                            />
                          ) : (
                            money(item.returnMinorUnits)
                          )}
                        </td>
                        <td className="px-4 py-3 text-xs font-semibold">
                          {compounds ? 'Roll into contribution' : 'Pay to wallet'}
                        </td>
                        <td className="px-4 py-3">
                          {ready ? money(item.principalAfterMinorUnits) : '—'}
                        </td>
                      </tr>
                    );
                  })}
            ...
            {editable && (
              <div className="flex flex-wrap items-center justify-between gap-3 border-t p-4">
                <p className="text-xs text-muted-foreground">
                  Enter the exact credited amount for every member, then save the manual returns.
                </p>
                <button
                  disabled={busy}
                  onClick={() => void saveManualReturns()}
