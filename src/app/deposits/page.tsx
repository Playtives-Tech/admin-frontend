...
import { MdCheckCircle, MdClose, MdContentCopy, MdImage, MdOpenInNew, MdPendingActions } from 'react-icons/md';
...
export default function DepositsPage(): React.JSX.Element {
  ...
  const load = () => getDepositRequests().then(setItems);
  useEffect(() => { void load().catch(() => notify.error('Could not load deposit requests')); }, []);
