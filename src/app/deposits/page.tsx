...
export default function DepositsPage(): React.JSX.Element {
  ...
  const review = async (status: 'approved' | 'rejected') => {
    ...
    try {
      ...
      setSelected(null);
      notify.success(
        status === 'approved' ? 'Deposit approved and wallet credited' : 'Deposit rejected',
      );
    } catch (error) {
          ...
          <div className="flex items-center justify-between gap-3 border-b px-4 py-3">
            <div>
              ...
              <p className="mt-0.5 text-xs text-muted-foreground">
                Automatically verified and already credited to member wallets.
              </p>
            </div>
            <span className="rounded-full bg-emerald-500/10 px-2.5 py-1 text-[10px] font-semibold text-emerald-700">
              {settledPaystack.length} settled
            </span>
          </div>
          <table className="w-full min-w-[720px] text-left text-xs">
            <thead className="border-b bg-muted/30 text-muted-foreground">
              <tr>
                {['Member', 'Amount', 'Channel', 'Reference', 'Credited'].map((label) => (
                  <th key={label} className="px-4 py-3 font-medium">
                    {label}
                  </th>
                ))}
            ...
            <tbody className="divide-y">
              {settledPaystack.map((item) => (
                <tr key={item._id} className="hover:bg-muted/20">
                  ...
                  <td className="px-4 py-3 text-muted-foreground">
                    {new Date(item.creditedAt ?? item.paidAt ?? item.createdAt).toLocaleString(
                      'en-NG',
                    )}
                  </td>
                </tr>
              ))}
              {settledPaystack.length === 0 ? (
                <tr>
...
function DepositReviewModal({
  ...
  reviewing: boolean;
  close: () => void;
  review: (status: 'approved' | 'rejected') => Promise<void>;
}) {
  const isPending = deposit.status === 'pending';
  const isPdf = /\.pdf(?:$|[?#])/i.test(deposit.receiptImageUrl);
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/45 p-4 backdrop-blur-sm">
      <section className="app-surface flex h-[min(60vh,680px)] w-full max-w-3xl flex-col overflow-hidden rounded-xl border">
        <header className="flex items-center justify-between border-b px-5 py-4">
          <div>
            <h2 className="text-sm font-semibold">Review deposit</h2>
            <p className="mt-1 text-xs text-muted-foreground">
              Confirm the receipt and amount before crediting.
            </p>
          </div>
          <button
            onClick={close}
        ...
        <div className="grid flex-1 gap-5 overflow-y-auto p-5 lg:grid-cols-[1.15fr_.85fr]">
          <div className="relative min-h-52 overflow-hidden rounded-lg border bg-muted">
            <a
              href={deposit.receiptImageUrl}
              target="_blank"
              rel="noreferrer"
              className="absolute right-2 top-2 z-10 grid size-8 place-items-center rounded-md bg-background/90 text-foreground"
            ...
            {isPdf ? (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="grid h-full min-h-52 place-items-center gap-2 p-6 text-center text-muted-foreground"
              >
                <MdPictureAsPdf className="size-10 text-red-500" />
                <span className="text-xs font-semibold text-foreground">PDF receipt</span>
                <span className="text-[11px]">Open the document to review it.</span>
              </a>
            ) : (
              <>
                {receiptLoading ? (
                  <div className="grid h-full min-h-52 place-items-center text-muted-foreground">
          ...
          <div className="space-y-4">
            <Detail label="Member" value={deposit.userId.name} helper={deposit.userId.email} />
            <Detail label="Amount" value={money(deposit.amountMinorUnits)} />
            <Detail label="Submitted" value={new Date(deposit.createdAt).toLocaleString('en-NG')} />
          ...
          {isPending ? (
            <>
              <button
                ...
                className="h-9 rounded-lg border border-red-500/30 px-3 text-xs font-semibold text-red-600 disabled:opacity-50"
              >
                Reject
              </button>
              <button
                ...
                className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-brand px-3 text-xs font-semibold text-brand-foreground disabled:opacity-50"
              >
                <MdCheckCircle className="size-4" />
                {reviewing ? 'Reviewing…' : 'Approve & credit'}
              </button>
            </>
          ) : null}
        </footer>
...
function Status({ status }: { status: AdminDepositRequest['status'] }) {
  return (
    <span
      className={`inline-flex rounded-full px-2 py-1 text-[10px] font-semibold ${status === 'approved' ? 'bg-emerald-500/10 text-emerald-700' : status === 'rejected' ? 'bg-red-500/10 text-red-700' : 'bg-amber-500/10 text-amber-700'}`}
    >
      {status}
    </span>
  );
