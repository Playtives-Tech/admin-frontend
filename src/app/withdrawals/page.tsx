...
const initialWithdrawals: WithdrawalRequest[] = [
  { id: 'WDR-001', user: 'Zainab Bello', email: 'zainab.b@example.com', amount: 250000, walletBalance: 150000, accrualsBalance: 300000, bankName: 'GTBank', accountNumber: '0123456789', accountName: 'Zainab Bello', date: 'Oct 15, 2026 14:30', status: 'Pending' },
  { id: 'WDR-002', user: 'Sarah Jenkins', email: 'sarah.j@example.com', amount: 500000, walletBalance: 150000, accrualsBalance: 600000, bankName: 'Access Bank', accountNumber: '0698765432', accountName: 'Sarah Jenkins', date: 'Oct 15, 2026 12:15', status: 'Pending' },
  { id: 'WDR-003', user: 'Michael Tosin', email: 'michael.t@example.com', amount: 1500000, walletBalance: 0, accrualsBalance: 1500000, bankName: 'Zenith Bank', accountNumber: '2023456789', accountName: 'Michael Tosin', date: 'Oct 14, 2026 09:45', status: 'Completed' },
  { id: 'WDR-004', user: 'David Olatunji', email: 'david.o@example.com', amount: 50000, walletBalance: 50000, accrualsBalance: 20000, bankName: 'First Bank', accountNumber: '3029876541', accountName: 'David Olatunji', date: 'Oct 14, 2026 16:20', status: 'Failed' },
];
              ...
              <tbody className="divide-y">
                {filteredWithdrawals.map((wdr) => (
                  <tr key={wdr.id} className="transition hover:bg-muted/30">
                    ...
                    <td className="px-6 py-4 text-right font-medium">₦{wdr.amount.toLocaleString()}</td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <span className={cn(
                        ...
                        wdr.status === 'Completed' ? 'bg-emerald-500/10 text-emerald-500' :
                        wdr.status === 'Pending' ? 'bg-amber-500/10 text-amber-500' :
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 p-4 backdrop-blur-sm">
          <div className="w-full max-w-lg overflow-hidden rounded-2xl border bg-surface shadow-xl">
            ...
            <div className="flex items-center justify-between border-b p-4">
              <div>
                <h2 className="font-heading text-lg font-semibold text-foreground">Review Withdrawal</h2>
              ...
              <div className="grid gap-6">
                ...
                <div className="grid grid-cols-2 gap-4 rounded-xl border bg-background p-4">
                  ...
                  <div>
                    ...
                    <p className="mt-1 font-heading text-2xl font-semibold text-brand">₦{selectedWithdrawal.amount.toLocaleString()}</p>
                  </div>
                </div>

                {/* User Balances Breakdown */}
                <div className="rounded-xl border bg-muted/30 p-4">
                  <p className="mb-3 text-xs font-bold uppercase tracking-wider text-muted-foreground">User Balances</p>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-xs text-muted-foreground">Deposited Funds</p>
                      <p className="font-medium">₦{selectedWithdrawal.walletBalance.toLocaleString()}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">Accruals (Returns)</p>
                      <p className="font-medium">₦{selectedWithdrawal.accrualsBalance.toLocaleString()}</p>
                    </div>
                  </div>
                  <div className="mt-3 border-t pt-3">
                    <p className="text-xs text-muted-foreground">Total Available</p>
                    <p className="font-medium text-brand">₦{(selectedWithdrawal.walletBalance + selectedWithdrawal.accrualsBalance).toLocaleString()}</p>
                  </div>
                </div>

                {/* Bank Details Card */}
                <div>
                  <p className="mb-2 text-xs font-bold uppercase tracking-wider text-muted-foreground">Destination Bank</p>
                  <div className="flex items-center gap-4 rounded-xl border bg-brand/5 p-4 text-brand">
                    ...
                    <div>
                      <p className="font-heading text-lg font-semibold">{selectedWithdrawal.bankName}</p>
                      <p className="text-sm font-medium">{selectedWithdrawal.accountNumber} · {selectedWithdrawal.accountName}</p>
                    </div>
                  </div>
                </div>
